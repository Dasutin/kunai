const input = document.getElementById('baseUrl');
const button = document.getElementById('save');
const message = document.getElementById('message');

const DEFAULT_BASE_URL = 'http://localhost:3000';

const cleanBaseUrl = (url) => {
  try {
    const parsed = new URL(url);
    parsed.pathname = parsed.pathname.replace(/\/$/, '');
    return parsed.toString().replace(/\/$/, '');
  } catch (err) {
    return null;
  }
};

const load = async () => {
  const { baseUrl = DEFAULT_BASE_URL } = await chrome.storage.sync.get(['baseUrl']);
  input.value = baseUrl;
};

const save = async () => {
  message.textContent = '';
  message.className = 'status';
  const url = input.value.trim() || DEFAULT_BASE_URL;
  const clean = cleanBaseUrl(url);
  if (!clean) {
    message.textContent = 'Enter a valid URL (e.g., https://example.com)';
    message.className = 'status error';
    return;
  }
  await chrome.storage.sync.set({ baseUrl: clean });
  chrome.runtime.sendMessage({ type: 'refresh-badge' }, (resp) => {
    if (chrome.runtime.lastError) {
      console.warn('Badge refresh message failed', chrome.runtime.lastError);
    }
    if (resp && resp.ok) {
      message.textContent = 'Saved. Badge refreshed.';
    } else {
      message.textContent = 'Saved. Badge will refresh shortly.';
    }
  });
};

button.addEventListener('click', save);
document.addEventListener('DOMContentLoaded', load);
